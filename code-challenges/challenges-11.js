...
function square(arr) {
        ...
        let resultArr = arr.map(valueIs => valueIs * valueIs)
        return resultArr;
...
function fullName(arr) {
    let fullName = arr.map(valueIs => `${valueIs.firstName} ${valueIs.lastName}`);
    return fullName;
...
function gradesAvg(arr) {
    let avgResult = arr.map((valueIs => ({ ...valueIs, avg: (valueIs.gradsList.reduce((x, y) => (x + y))) / valueIs.gradsList.length })));
...
function studentsResult(arr) {
    const result = arr.map((valueIs => ({
        ...valueIs, ...(((valueIs.gradsList.reduce((x1, y1) => x1 + y1)) / valueIs.gradsList.length) >= 50 && { result: "Passed" }),
        ...(((valueIs.gradsList.reduce((x1, y1) => x1 + y1)) / valueIs.gradsList.length) < 50 && { result: "Failed" }),
    })))
    return result;
